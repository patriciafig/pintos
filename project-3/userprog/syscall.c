...
read (int fd, void *buffer, unsigned size) {
  ...
  while (buffer_tmp != NULL) {
    ...
    if (pagedir_get_page(thread_current()->pagedir, buffer_tmp) == NULL) {
      struct suppl_pte *spte = get_suppl_pte (&thread_current()->suppl_page_table, pg_round_down (buffer_tmp));
...
int write(int fd, const void *buffer, unsigned size) {
  ...
  // determine if the pointed memory is valid
  while (buffer_tmp != NULL) {
...
// add an entry in the mmfiles table and associated entiries in the suppl. page table
mapid_t mmap (int fd, void *addr) {
  // do not map file to memory if these conditions hold true
  ...
  uint32_t len = file_length(fd_struct->file_struct);
  if (len <= 0) {
    return -1;
  }
  ...
  int offset = 0;
  while (offset < len) {
    if (get_suppl_pte(&thread_current()->suppl_page_table, addr + offset)) {
      return -1;
    }
...
// remove an entry in the mmfiles table and associated entiries in the suppl. page table
